    ...
    for (let i = 0; len = squares.length; i++)
        ...
        (function(index) {
            ...
            squares[i].onclick = function(){
                //if square below the selected square is taken you can play this square
        ...
        function checkBoard() {
            //found code for all winning arrays
            const winningArrays = [
                ...
                for (let y = 0; y < winningArrays.length; y++) {
                    ...
                    // check arrays for class name player 1
                    if (square1.classList.contains('player-one') &&
                        square2.classList.contains('player-one') &&
                        square3.classList.contains('player-one') &&
                        square4.classList.contains('player-one')) {
                            //if yes, player 1 wins
                            result.innerHTML = 'Player 1 wins!'
                    }
                    //check arrays for class name player 2
                    if (square1.classList.contains('player-two') &&
                    square2.classList.contains('player-two') &&
                    square3.classList.contains('player-two') &&
                    square4.classList.contains('player-two')) {
                        // if yes, player 2 wins
                        result.innerHTML = 'Player 2 wins!'
                    }
                }
            }

    // add eventListener to each square that will trigger the checkBoard function on each click
